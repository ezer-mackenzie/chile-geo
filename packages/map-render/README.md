...
Use `@chile-geo/maps/2d` for the smallest Canvas-only dependency graph. Use `@chile-geo/maps/3d` after installing the `three` peer dependency. Both classes accept `updateData(RegionData[])` and expose `destroy()` for lifecycle cleanup.
